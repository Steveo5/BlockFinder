...
public class CommandListener implements CommandExecutor {
	...
	public CommandListener(BlockFinder plugin)
	{
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length > 0 && args[0].equalsIgnoreCase("give")) {
			...
			OfflinePlayer sendTo = null;
			if (!(sender instanceof Player)) {
				if (args.length > 1) {
					sendTo = Bukkit.getOfflinePlayer(args[1]);
					System.out.println("Gtting " + args[1]);
					if (sendTo == null) {
						sender.sendMessage(ChatColor.RED + "You must enter a valid player name");
						return true;
					}
			...
			StringBuilder builder = new StringBuilder();
			builder.append("&a&lCollected &e&l" + BlockFinder.getFound(sendTo.getUniqueId()).size() + " / " + BlockFinder.getBlockFinds().size() + " &a&lpieces");
			for (BlockFind find : BlockFinder.getBlockFinds()) {
				if (BlockFinder.getFound(sendTo.getUniqueId()).contains(find.getName())) {
		...
		{
			plugin.loadSettings();
			BlockFinder.getDataSource().reload();
			sender.sendMessage(StringUtil.color("&aConfig has been reloaded successfully"));
		} else
		{
			...
			builder.append("\n&a/bf reload - &ereload all spawns from config");
			sender.sendMessage(StringUtil.color(builder.toString()));
			return true;
		}
		return false;
